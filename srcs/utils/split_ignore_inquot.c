					...
					const char *delimiters,t_splvariab *varia)
{
	...
	*next_str_len = 0;
	...
	{ 
		k=(*next_str)[i];
		if (che_open_quo(varia, k)  && ft_strchr(delimiters, (*next_str)[i]))
			return ;
		(*next_str_len)++;
		i++;
	}
...
char	**ft_split_igquo(char const *s, const char *delimiters)
{
	...
	variabl.double_q=0;
	variabl.single_q=0;
	nb_strs = ft_get_splnb(s, delimiters,&variabl);
	...
	while (i < nb_strs)
	{
		ft_get_next_str1(&next_str, &next_str_len, delimiters,&variabl);
		tab[i] = malloc(sizeof(char) * (next_str_len + 1));
		if (tab[i] == NULL)
			return (ft_clsplit(tab));
